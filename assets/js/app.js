...
function updateClock() {
  const now = new Date();
  const lang = language();
  document.querySelector('#local-clock').textContent = new Intl.DateTimeFormat(lang.locale, { hour: '2-digit', minute: '2-digit', second: '2-digit' }).format(now);
  document.querySelector('#local-date').textContent = new Intl.DateTimeFormat(lang.locale, { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }).format(now);
  document.querySelector('#local-zone').textContent = Intl.DateTimeFormat().resolvedOptions().timeZone;
  document.querySelector('#utc-clock').textContent = new Intl.DateTimeFormat('en-GB', { timeZone: 'UTC', hour: '2-digit', minute: '2-digit', hour12: false }).format(now);
}
