...
export function renderWeek({ container, events, weekStart, selectedDate, onSelectDate, language }) {
  ...
  for (let i = 0; i < 7; i += 1) {
    ...
    heading.innerHTML = `<strong>${formatDayTitle(date, language.locale)}</strong><span>${formatDayNumber(date, language.locale)}</span>`;
    column.appendChild(heading);
    const dayEvents = events.filter(event => isSameDay(event.start, date));
    const logos = document.createElement('div'); logos.className = 'day-column__logos';
    uniqueChampionships(dayEvents).forEach(event => logos.appendChild(logoElement(event, 'week-logo')));
...
export function renderDayProgram({ container, events, selectedDate, language }) {
  ...
  table.innerHTML = `<div class="program-row program-row--head"><div>${c.time}</div><div>${c.championship}</div><div>${c.event}</div><div>${c.session}</div><div>${c.location}</div><div>${c.status}</div></div>`;
  dayEvents.forEach(event => {
    ...
    const cells = [
      ...
      `<div class="program-event">${translatedEventName(event, language)}</div>`,
